...
echo "[*] Updating cloudfront_domain variable in /tools/megazord-composition/.env"
original_val=$(grep 'CLOUDFRONT_DOMAIN' /tools/megazord-composition/.env)

if ! sed -i "s|${original_val}|CLOUDFRONT_DOMAIN=${cloudfront_domain}|" \
  /tools/megazord-composition/.env; then
  echo -e "${RED_FG}[ \U2757] Error updating CLOUDFRONT_DOMAIN in /tools/megazord-composition/.env${RESET}"
  exit 1
fi

echo -e "${GREEN_FG}[\U2714] Added CLOUDFRONT_DOMAIN to .env${RESET}\n"

# Add c2_domain to env file

echo "[*] Updating C2_DOMAIN variable in /tools/megazord-composition/.env"
original_val=$(grep 'C2_DOMAIN' /tools/megazord-composition/.env)

if ! sed -i "s|${original_val}|C2_DOMAIN=${domain}|" \
  /tools/megazord-composition/.env; then
  echo -e "${RED_FG}[ \U2757] Error updating C2_DOMAIN in /tools/megazord-composition/.env${RESET}"
  exit 1
fi

echo -e "${GREEN_FG}[\U2714] Added C2_DOMAIN to .env${RESET}\n"
...
cna_dir=/tools/megazord-composition/cnas/server_cnas/

# Update c2_domain, cloudfront_domain, and payload_dir variables in megazord.cna

echo "[*] Updating cloudfront_domain variable in megazord.cna"
original_val=$(grep 'cloudfront_domain = \"' /tools/megazord-composition/cnas/server_cnas/megazord.cna)

if ! sed -i "s|${original_val}|\$cloudfront_domain = \"${cloudfront_domain}\";|" \
  ${cna_dir}megazord.cna; then
  echo -e "${RED_FG}[ \U2757] Error updating cloudfront_domain in megazord.cna${RESET}"
  exit 1
fi

echo -e "${GREEN_FG}[\U2714] Updated cloudfront_domain in megazord.cna${RESET}\n"

echo "[*] Updating c2_domain variable in megazord.cna"
original_val=$(grep 'c2_domain =' ${cna_dir}megazord.cna)
if ! sed -i "s|${original_val}|\$c2_domain = \"${domain}\";|" \
  ${cna_dir}megazord.cna; then
  echo -e "${RED_FG}[ \U2757] Error updating c2_domain in megazord.cna${RESET}"
  exit 1
fi

echo -e "${GREEN_FG}[\U2714] Updated c2_domain in megazord.cna${RESET}\n"

echo "[*] Updating payload_dir variable in megazord.cna"
original_val=$(grep 'payload_dir = \"' ${cna_dir}megazord.cna)
if ! sed -i "s|${original_val}|\$payload_dir = \"${endpoint}\";|" \
  ${cna_dir}megazord.cna; then
  echo -e "${RED_FG}[ \U2757] Error updating payload_dir in megazord.cna${RESET}"
  exit 1
fi

echo -e "${GREEN_FG}[\U2714] Updated payload_dir in megazord.cna${RESET}\n"
